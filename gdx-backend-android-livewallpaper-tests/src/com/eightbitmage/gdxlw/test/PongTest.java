...
public class PongTest implements ApplicationListener, InputProcessor {
	...
	/**
	 * Here we setup all the resources. A MeshRenderer for the paddles which we
	 * use for both, a MeshRenderer for the ball and a Text for rendering the
	 * score.
	 */
	@Override
	public void create() {
	...
	/**
	 * This method sets up all the graphics related stuff like the Meshes, the
	 * camera and the Font
	 */
	private void setupGraphics() {
		...
		//
		paddleMesh = new Mesh(true, 4, 0, new VertexAttribute(Usage.Position,
				2, "a_position"));
		...
		//
		ballMesh = new Mesh(true, 4, 0, new VertexAttribute(Usage.Position, 2,
				"a_position"));
	...
	/**
	 * This sets up the game data like the initial paddle positions and the ball
	...
	public void render() {
		...
		spriteBatch.begin();
		// spriteBatch.drawText(font, score, Gdx.graphics.getWidth() / 2 -
		// font.getStringWidth(score) / 2, Gdx.graphics.getHeight()
		// - font.getLineHeight(), Color.WHITE);
		spriteBatch.end();
	}

	/**
	 * Updates the game state, moves the ball, checks for collisions or whether
	 * the ball has left the playfield.
	 */
	private void updateGame() {
		...
		ball.add(ballSpeed * ballDirection.x * deltaTime, ballSpeed
				* ballDirection.y * deltaTime);
		...
		if (ball.x < -240) {
			...
			ballDirection.set((float) Math.random() + 0.1f,
					(float) Math.random()).nor(); // new ball direction, must be
													// unit length
		...
		if (ballDirection.x > 0 && ball.x > rightPaddle.x - 5
				&& ball.x < rightPaddle.x + 5 && ball.y > rightPaddle.y - 30
				&& ball.y < rightPaddle.y + 30) {
			ball.x = rightPaddle.x - 6; // set the position of a little so we
										// don't get to this code in the next
										// frame
			ballDirection.x = -ballDirection.x;
			float sign = Math.signum(ball.y - rightPaddle.y);
			ballDirection.y = sign * (float) Math.abs(ball.y - rightPaddle.y)
					/ 30; // reflect it depending on where the paddle was
			// hit
			ballDirection.nor();
		...
		if (ballDirection.x < 0 && ball.x < leftPaddle.x + 5
				&& ball.x > leftPaddle.x - 5 && ball.y > leftPaddle.y - 30
				&& ball.y < leftPaddle.y + 30) {
			ball.x = leftPaddle.x + 6; // set the position of a little so we
										// don't get to this code in the next
										// frame
			ballDirection.x = -ballDirection.x;
			float sign = (float) Math.signum(ball.y - leftPaddle.y);
			ballDirection.y = sign * (float) Math.abs(ball.y - leftPaddle.y)
					/ 30; // reflect it depending on where the paddle was hit
			ballDirection.nor();
			ballSpeed += 10; // and faster!
			if (ballSpeed > 300)
				ballSpeed = 300;
		...
		// float touchX = 480 * (Gdx.input.getX() /
		// (float)Gdx.graphics.getWidth() - 0.5f);
		// float touchY = 320 * (0.5f - Gdx.input.getY() /
		...
		if (ballDirection.x > 0) {
			float dir = (float) Math.signum(ball.y - rightPaddle.y);
			rightPaddle.y += dir * deltaTime * (ballSpeed * leftPaddleMulti);
			if (dir > 0 && rightPaddle.y > ball.y)
		...
		if (displayFPS++ > 100) {
			Log.d("PongTest", "FPS-" + Gdx.graphics.getFramesPerSecond());
			displayFPS = 0;
		}
	}
